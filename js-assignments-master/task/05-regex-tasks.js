...
function getRegexForGuid() {
   var regexp=/^{[0-9a-f-A-F]{8}-[0-9a-f-A-F]{4}-[0-9a-f-A-F]{4}-[0-9a-f-A-F]{4}-[0-9a-f-A-F]{12}}$/;
   return regexp;
}
...
function getRegexForSSN() {
   var regexp=/^(?!000)\d{3}-(?!00)\d{2}-(?!0000)\d{4}$/;
   return regexp;
}
...
function getPasswordValidator(minLength) {
   ...
   return new RegExp('^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[0a-zA-Z0-9]{'+minLength+',}$');
}
