...
function doRectanglesOverlap(rect1, rect2) {
  var lengthHeight = rect1.top + rect1.height;
  var lengthWidth = rect1.left + rect1.width;
  if ((lengthHeight - rect2.top > 0) && (lengthWidth - rect2.left > 0)) return true;
  else return false;
}
...
function isInsideCircle(circle, point) {
  ...
  if (circle.radius > r) return true;
  else return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    ...
    if (a<b) {str+=a; str+=", "; str+=b;}
    else {str+=b; str+=", "; str+=a;}
...
var start=new Date(startDate);
  var end=new Date(endDate);
    ...
    var time=end-start; var str="";
    if (time<45) str="a few";
    if (time>=45 && time <90) str="minute";
    if (time>=90 && time<(45*60)) {str=Math.round(time/60)+" minutes";}
    if (time>=(45*60) && time<(90*60)) str="an hour";
    if (time>=(90*60) && time<(22*60*60)) {str=Math.round(time/3600)+" hours";}
...
function getCommonDirectoryPath(pathes) {
  var arr=new Array(pathes.length).fill(0);
  for (var i=0; i<pathes.length; i++)
    ...
    if (count==0) answer.push(arr[0][j]);
    j++;
  }
  answer.push("");
  return answer.join("/");
    ...
    for (var i=0; i<3; i++)
    {
       ...
       if ((position[0][i]==position[1][i]) && (position[1][i]==position[2][i])){
         if (position[0][i]=='X') return x;
           else if (position[0][i]=='0') return o;
         }
    }
   ...
   for (var i=0; i<3; i++)
   {
      if ((position[i][0]==position[i][1]) && (position[i][1]==position[i][2])){
        if (position[i][0]=='X') return x;
          else if (position[i][0]=='0') return o;
          }
        }
